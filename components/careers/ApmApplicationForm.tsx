...
export default function ApmApplicationForm() {
  ...
  async function onSubmit(values: FormFields) {
    ...
    const parsed = apmApplicationSchema.safeParse({
      ...values,
      sourceUrl,
    })
    ...
    try {
      ...
      fd.append('automationOperations', d.automationOperations)
      fd.append('taskPipelineApproach', d.taskPipelineApproach)
      ...
      const res = await fetch('/api/careers-apm', {
        method: 'POST',
        body: fd,
      })
    ...
    return (
      <div className="rounded-xl border border-[#ceead6] bg-[#e6f4ea] p-8 text-center">
        ...
        <p className="mt-3 text-sm leading-relaxed text-[#3c4043]">
          Thank you. If there is a fit, we will get back within about two weeks. Check spam folders
          just in case.
        </p>
        <button
          type="button"
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-7 font-sans">
      <div className="grid gap-6 sm:grid-cols-2">
        <div>
          <label htmlFor="apm-fullName" className={labelClass}>
        ...
        <div>
          <label htmlFor="apm-email" className={labelClass}>
      ...
      <div>
        <label htmlFor="apm-phone" className={labelClass}>
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        <div>
          <label htmlFor="apm-linkedinUrl" className={labelClass}>
        ...
        <div>
          <label htmlFor="apm-portfolioUrl" className={labelClass}>
      ...
      <div>
        <label htmlFor="apm-resume" className={labelClass}>
          CV or résumé <span className="font-normal text-[#80868b]">(optional but recommended)</span>
        </label>
      ...
      <div>
        <label htmlFor="apm-relevantExperience" className={labelClass}>
          Relevant experience <span className="text-[#d93025]">*</span>
        </label>
        <p className="mt-1 text-xs text-[#5f6368]">
          Your background, domains you have worked in, and why this role interests you (a short
          paragraph is fine).
        </p>
        <textarea
          id="apm-relevantExperience"
      ...
      <div>
        <label htmlFor="apm-automationOperations" className={labelClass}>
          How would you automate operations at EzyHelpers? <span className="text-[#d93025]">*</span>
        </label>
        <p className="mt-1 text-xs text-[#5f6368]">
          Domestic helper platform: matching, sourcing, field ops, CX, dashboards — be specific.
        </p>
        <textarea
          id="apm-automationOperations"
          {...register('automationOperations')}
          rows={8}
          className={`${inputClass} resize-y leading-relaxed`}
        />
        {err('automationOperations') && (
          <p className="mt-1.5 text-sm text-[#d93025]">{err('automationOperations')}</p>
        )}
      </div>

      <div>
        <label htmlFor="apm-taskPipelineApproach" className={labelClass}>
          How would you keep a dev team always supplied with clear next work?{' '}
          <span className="text-[#d93025]">*</span>
        </label>
        <p className="mt-1 text-xs text-[#5f6368]">
          This role owns the task pipeline — prioritisation, PRDs, and avoiding idle build time.
        </p>
        <textarea
          id="apm-taskPipelineApproach"
          {...register('taskPipelineApproach')}
          rows={5}
          className={`${inputClass} resize-y leading-relaxed`}
        />
        {err('taskPipelineApproach') && (
          <p className="mt-1.5 text-sm text-[#d93025]">{err('taskPipelineApproach')}</p>
        )}
      </div>

      <div>
        <label htmlFor="apm-additionalNotes" className={labelClass}>
          Anything else? <span className="font-normal text-[#80868b]">(optional)</span>
        </label>
        <textarea
          id="apm-additionalNotes"
