export type Language = 'en' | 'hi' | 'kn' | 'ta' | 'te';

export const translations: Record<Language, any> = {
    en: {
        label: 'English',
        title: 'HELPER INTERVIEW QUESTIONNAIRE',
        sections: {
            basic: '1. Basic Information',
            workPrefs: '2. Work Preferences',
            habits: '3. Work Habits & Discipline',
            health: '4. Health & Leave Policy',
            lifestyle: '5. Food & Lifestyle Preferences',
            training: '6. Training & Skills',
            personal: '7. Personal Habits',
            consent: 'Legal Consent'
        },
        fields: {
            fullName: 'Full Name',
            age: 'Age',
            maritalStatus: 'Marital Status (Married / Unmarried / Widow / Widower)',
            hasChildren: 'Do you have children? If yes, how many and what are their ages?',
            localReference: 'Do you have any local reference? If yes, please share the contact number',
            hasSmartphone: 'Do you have a smartphone?',
            speakHindi: 'Can you speak Hindi?',
            speakOtherLanguages: 'Can you speak English or any other language? If yes, please specify',
            readWriteHindi: 'Can you read and write Hindi?',
            workInterests: 'What type of work are you interested in? (Cooking / Housekeeping / Babysitting / Elderly Care / All-rounder)',
            comfortableCooking: 'Are you comfortable with cooking?',
            cookType: 'Do you cook vegetarian, non-vegetarian, or both?',
            prepareMilletDietary: 'Can you prepare millet or dietary meals?',
            comfortableHousekeeping: 'Are you comfortable with housekeeping (cleaning, mopping, dusting)?',
            willingCleanBathrooms: 'Are you willing to clean bathrooms?',
            knowIroning: 'Do you know how to iron clothes?',
            operateWashingMachine: 'Can you operate a washing machine?',
            useAppliances: 'Do you know how to use a microwave, mixer, oven, or air fryer?',
            comfortableKids: 'Are you comfortable taking care of children?',
            comfortableElderly: 'Are you comfortable handling elderly people?',
            dropChildrenBusStop: 'Are you okay with dropping children at the bus stop?',
            canDrive: 'Can you drive a car or two-wheeler?',
            wakeUpTime: 'What time do you usually wake up?',
            earlyStart: 'Are you comfortable starting work early in the morning?',
            comfortablePets: 'Are you comfortable with pets (especially dogs)?',
            nonVegHome: 'Are you comfortable working in a home where non-vegetarian food is cooked?',
            phoneDuringWork: 'Do you use your phone during work hours?',
            socialMediaWork: 'Are you active on social media during work time?',
            manageMultipleTasks: 'Are you able to manage multiple tasks at the same time?',
            runBehindChildren: 'Are you comfortable running behind small children if required?',
            regularMedicines: 'Do you take any regular medicines?',
            healthConditions: 'Do you have any health conditions we should know about?',
            monthlyLeave: 'Do you need monthly leave? If yes, how many days?',
            leavePreference: 'Do you prefer taking leave monthly or all together?',
            restTimeNeeded: 'Do you need rest time during work hours? If yes, how long?',
            menstrualCycleComfort: 'Are you comfortable working during your menstrual cycle? (Optional)',
            eatSameFood: 'Are you comfortable eating the same food prepared in the house?',
            vegNonVegPreference: 'Are you okay with eating vegetarian or non-vegetarian food based on the household?',
            foodRestrictions: 'Do you have any food restrictions?',
            formalTraining: 'Have you received any formal training in housekeeping or elderly care?',
            specificTrainingNeeded: 'Do you need any specific training?',
            openToTraining: 'Are you open to receiving additional training if required?',
            personalHabits: 'Do you smoke, drink alcohol, or use tobacco?',
            followHouseRules: 'Are you willing to follow the employer’s house rules?',
            consentText: 'I hereby confirm that all the information provided by me is true and correct to the best of my knowledge. I understand that providing false information may lead to rejection or termination of employment. I voluntarily consent to share my personal details for verification and placement purposes.'
        }
    },
    hi: {
        label: 'हिंदी',
        title: 'हेल्पर इंटरव्यू प्रश्नावली',
        sections: {
            basic: '1. बुनियादी जानकारी',
            workPrefs: '2. कार्य प्राथमिकताएँ',
            habits: '3. कार्य आदतें और अनुशासन',
            health: '4. स्वास्थ्य और अवकाश नीति',
            lifestyle: '5. भोजन और जीवनशैली',
            training: '6. प्रशिक्षण और कौशल',
            personal: '7. व्यक्तिगत आदतें',
            consent: 'कानूनी सहमति'
        },
        fields: {
            fullName: 'पूरा नाम',
            age: 'आयु',
            maritalStatus: 'वैवाहिक स्थिति (विवाहित / अविवाहित / विधवा / विधुर)',
            hasChildren: 'क्या आपके बच्चे हैं? यदि हाँ, तो कितने और उनकी उम्र क्या है?',
            localReference: 'क्या आपके पास कोई स्थानीय संदर्भ है? यदि हाँ, तो संपर्क नंबर बताएं',
            hasSmartphone: 'क्या आपके पास स्मार्टफोन है?',
            speakHindi: 'क्या आप हिंदी बोल सकते / सकती हैं?',
            speakOtherLanguages: 'क्या आप अंग्रेज़ी या कोई अन्य भाषा बोल सकते / सकती हैं? यदि हाँ, तो बताएं',
            readWriteHindi: 'क्या आप हिंदी पढ़ और लिख सकते / सकती हैं?',
            workInterests: 'आप किस प्रकार का काम करना चाहते / चाहती हैं? (खाना बनाना / घर की सफ़ाई / बच्चों की देखभाल / बुज़ुर्गों की देखभाल / सभी काम)',
            comfortableCooking: 'क्या आप खाना बनाने में सहज हैं?',
            cookType: 'क्या आप शाकाहारी, मांसाहारी या दोनों प्रकार का खाना बना सकते / सकती हैं?',
            prepareMilletDietary: 'क्या आप मिलेट या डाइट फूड बना सकते / सकती हैं?',
            comfortableHousekeeping: 'क्या आप झाड़ू, पोछा और डस्टिंग जैसे काम कर सकते / सकती हैं?',
            willingCleanBathrooms: 'क्या आप बाथरूम साफ़ करने के लिए तैयार हैं?',
            knowIroning: 'क्या आप कपड़े इस्त्री करना जानते / जानती हैं?',
            operateWashingMachine: 'क्या आप वॉशिंग मशीन चला सकते / सकती हैं?',
            useAppliances: 'क्या आप माइक्रोवेव, मिक्सर, ओवन या एयर फ्रायर का उपयोग करना जानते / जानती हैं?',
            comfortableKids: 'क्या आप बच्चों की देखभाल कर सकते / सकती हैं?',
            comfortableElderly: 'क्या आप बुज़ुर्गों की देखभाल कर सकते / सकती हैं?',
            dropChildrenBusStop: 'क्या आप बच्चों को बस स्टॉप तक छोड़ सकते / सकती हैं?',
            canDrive: 'क्या आप कार या दोपहिया वाहन चला सकते / सकती हैं?',
            wakeUpTime: 'आप आमतौर पर कितने बजे उठते / उठती हैं?',
            earlyStart: 'क्या आप सुबह जल्दी काम शुरू करने में सहज हैं?',
            comfortablePets: 'क्या आप पालतू जानवरों, विशेषकर कुत्तों के साथ सहज हैं?',
            nonVegHome: 'क्या आप ऐसे घर में काम कर सकते / सकती हैं जहाँ मांसाहारी भोजन बनता हो?',
            phoneDuringWork: 'क्या आप काम के समय मोबाइल फोन का उपयोग करते / करती हैं?',
            socialMediaWork: 'क्या आप काम के समय सोशल मीडिया का उपयोग करते / करती हैं?',
            manageMultipleTasks: 'क्या आप एक साथ कई काम संभाल सकते / सकती हैं?',
            runBehindChildren: 'क्या ज़रूरत पड़ने पर आप छोटे बच्चों के पीछे दौड़ सकते / सकती हैं?',
            regularMedicines: 'क्या आप कोई नियमित दवाइयाँ लेते / लेती हैं?',
            healthConditions: 'क्या आपको कोई स्वास्थ्य समस्या है जिसकी जानकारी हमें होनी चाहिए?',
            monthlyLeave: 'क्या आपको मासिक अवकाश चाहिए? यदि हाँ, तो कितने दिन?',
            leavePreference: 'क्या आप अवकाश मासिक लेना पसंद करते / करती हैं या एक साथ?',
            restTimeNeeded: 'क्या आपको काम के दौरान आराम का समय चाहिए? यदि हाँ, तो कितनी देर?',
            menstrualCycleComfort: 'क्या आप मासिक धर्म के दौरान काम करने में सहज हैं? (वैकल्पिक)',
            eatSameFood: 'क्या आप घर में बना वही भोजन खाने में सहज हैं?',
            vegNonVegPreference: 'क्या आप घर के अनुसार शाकाहारी या मांसाहारी भोजन खाने में सहज हैं?',
            foodRestrictions: 'क्या आपको किसी प्रकार की भोजन संबंधी पाबंदी है?',
            formalTraining: 'क्या आपने हाउसकीपिंग या बुज़ुर्गों की देखभाल का कोई औपचारिक प्रशिक्षण लिया है?',
            specificTrainingNeeded: 'क्या आपको किसी विशेष प्रशिक्षण की आवश्यकता है?',
            openToTraining: 'क्या आप आवश्यकता होने पर अतिरिक्त प्रशिक्षण लेने के लिए तैयार हैं?',
            personalHabits: 'क्या आप धूम्रपान, शराब या तंबाकू का सेवन करते / करती हैं?',
            followHouseRules: 'क्या आप नियोक्ता के घर के नियमों का पालन करने के लिए तैयार हैं?',
            consentText: 'मैं यह पुष्टि करता / करती हूँ कि मेरे द्वारा दी गई सभी जानकारी सत्य है। मैं सत्यापन और प्लेसमेंट के उद्देश्य से अपनी व्यक्तिगत जानकारी साझा करने की सहमति देता / देती हूँ।'
        }
    },
    kn: {
        label: 'ಕನ್ನಡ',
        title: 'ಹೆಲ್ಪರ್ ಸಂದರ್ಶನ ಪ್ರಶ್ನಾವಳಿ',
        sections: {
            basic: '1. ಮೂಲ ಮಾಹಿತಿ',
            workPrefs: '2. ಕೆಲಸದ ಆದ್ಯತೆಗಳು',
            habits: '3. ಕೆಲಸದ ಅಭ್ಯಾಸ ಮತ್ತು ಶಿಸ್ತು',
            health: '4. ಆರೋಗ್ಯ ಮತ್ತು ರಜೆ',
            lifestyle: '5. ಆಹಾರ ಮತ್ತು ಜೀವನಶೈಲಿ',
            training: '6. ತರಬೇತಿ ಮತ್ತು ಕೌಶಲ್ಯ',
            personal: '7. ವೈಯಕ್ತಿಕ ಅಭ್ಯಾಸಗಳು',
            consent: 'ಕಾನೂನು ಒಪ್ಪಿಗೆ'
        },
        fields: {
            fullName: 'ಪೂರ್ಣ ಹೆಸರು',
            age: 'ವಯಸ್ಸು',
            maritalStatus: 'ವೈವಾಹಿಕ ಸ್ಥಿತಿ (ವಿವಾಹಿತ / ಅವಿವಾಹಿತ / ವಿಧವೆ / ವಿಧುರ)',
            hasChildren: 'ನಿಮಗೆ ಮಕ್ಕಳು ಇದೆಯೆ? ಇದ್ದರೆ ಎಷ್ಟು ಮತ್ತು ಅವರ ವಯಸ್ಸು ಎಷ್ಟು?',
            localReference: 'ನಿಮಗೆ ಸ್ಥಳೀಯ ಪರಿಚಯ ಇದೆಯೆ? ಇದ್ದರೆ ಸಂಪರ್ಕ ಸಂಖ್ಯೆ ನೀಡಿ',
            hasSmartphone: 'ನಿಮ್ಮ ಬಳಿ ಸ್ಮಾರ್ಟ್ಫೋನ್ ಇದೆಯೆ?',
            speakHindi: 'ನೀವು ಹಿಂದಿ ಮಾತನಾಡಬಹುದೇ?',
            speakOtherLanguages: 'ನೀವು ಇಂಗ್ಲಿಷ್ ಅಥವಾ ಬೇರೆ ಭಾಷೆ ಮಾತನಾಡಬಹುದೇ? ಇದ್ದರೆ ತಿಳಿಸಿ',
            readWriteHindi: 'ನೀವು ಹಿಂದಿ ಓದಲು ಮತ್ತು ಬರೆಯಲು ಬಲ್ಲಿರಾ?',
            workInterests: 'ನೀವು ಯಾವ ರೀತಿಯ ಕೆಲಸ ಮಾಡಲು ಆಸಕ್ತರಾಗಿದ್ದೀರಿ? (ಅಡುಗೆ / ಮನೆ ಸ್ವಚ್ಛತೆ / ಮಕ್ಕಳ ಆರೈಕೆ / ಹಿರಿಯರ ಆರೈಕೆ / ಎಲ್ಲ ಕೆಲಸ)',
            comfortableCooking: 'ಅಡುಗೆ ಮಾಡಲು ನೀವು ಸುಲಭವಾಗಿದ್ದೀರಾ?',
            cookType: 'ನೀವು ಶಾಕಾಹಾರಿ, ಮಾಂಸಾಹಾರಿ ಅಥವಾ ಎರಡೂ ರೀತಿಯ ಆಹಾರ ತಯಾರಿಸಬಹುದೇ?',
            prepareMilletDietary: 'ಮಿಲ್ಲೆಟ್ ಅಥವಾ ಡೈಟ್ ಆಹಾರ ತಯಾರಿಸಬಹುದೇ?',
            comfortableHousekeeping: 'ಜಾಡು, ಪೊಚ್ಚು ಮತ್ತು ಧೂಳು ತೂಗುವ ಕೆಲಸ ಮಾಡಬಹುದೇ?',
            willingCleanBathrooms: 'ಬಾತ್ರೂಮ್ ಸ್ವಚ್ಛಗೊಳಿಸಲು ಸಿದ್ಧರಿದ್ದೀರಾ?',
            knowIroning: 'ಬಟ್ಟೆ ಇಸ್ತ್ರಿ ಮಾಡಬಹುದೇ?',
            operateWashingMachine: 'ವಾಷಿಂಗ್ ಮೆಷಿನ್ ಬಳಸಬಹುದೇ?',
            useAppliances: 'ಮೈಕ್ರೋವೇವ್, ಮಿಕ್ಸರ್, ಓವನ್ ಅಥವಾ ಏರ್ ಫ್ರೈಯರ್ ಬಳಸಬಹುದೇ?',
            comfortableKids: 'ಮಕ್ಕಳನ್ನು ನೋಡಿಕೊಳ್ಳಲು ಸಿದ್ಧರಿದ್ದೀರಾ?',
            comfortableElderly: 'ಹಿರಿಯ ನಾಗರಿಕರ ಆರೈಕೆ ಮಾಡಲು ಸಿದ್ಧರಿದ್ದೀರಾ?',
            dropChildrenBusStop: 'ಮಕ್ಕಳನ್ನು ಬಸ್ ಸ್ಟಾಪ್ಗೆ ಬಿಡಲು ಸಿದ್ಧರಿದ್ದೀರಾ?',
            canDrive: 'ನೀವು ಕಾರ್ ಅಥವಾ ದ್ವಿಚಕ್ರ ವಾಹನ ಚಲಾಯಿಸಬಹುದೇ?',
            wakeUpTime: 'ನೀವು ಸಾಮಾನ್ಯವಾಗಿ ಎಷ್ಟು ಗಂಟೆಗೆ ಎದ್ದುಕೊಳ್ಳುತ್ತೀರಿ?',
            earlyStart: 'ಬೆಳಿಗ್ಗೆ ಬೇಗ ಕೆಲಸ ಆರಂಭಿಸಲು ಸಿದ್ಧರಿದ್ದೀರಾ?',
            comfortablePets: 'ಪೆಟ್ಸ್, ವಿಶೇಷವಾಗಿ ನಾಯಿಗಳೊಂದಿಗೆ ನಿಮಗೆ ಅನುಕೂಲವೇ?',
            nonVegHome: 'ಮಾಂಸಾಹಾರಿ ಆಹಾರ ತಯಾರಿಸುವ ಮನೆಯಲ್ಲಿ ಕೆಲಸ ಮಾಡಲು ಸಿದ್ಧರಿದ್ದೀರಾ?',
            phoneDuringWork: 'ಕೆಲಸದ ಸಮಯದಲ್ಲಿ ಮೊಬೈಲ್ ಬಳಸುತ್ತೀರಾ?',
            socialMediaWork: 'ಕೆಲಸದ ವೇಳೆ ಸಾಮಾಜಿಕ ಮಾಧ್ಯಮ ಬಳಸುತ್ತೀರಾ?',
            manageMultipleTasks: 'ಒಂದೇ ಸಮಯದಲ್ಲಿ ಹಲವಾರು ಕೆಲಸ ನಿರ್ವಹಿಸಬಹುದೇ?',
            runBehindChildren: 'ಅಗತ್ಯವಿದ್ದರೆ ಮಕ್ಕಳ ಹಿಂದೆ ಓಡಲು ಸಿದ್ಧರಿದ್ದೀರಾ?',
            regularMedicines: 'ನೀವು ನಿಯಮಿತವಾಗಿ ಯಾವುದೇ ಔಷಧಿ ತೆಗೆದುಕೊಳ್ಳುತ್ತೀರಾ?',
            healthConditions: 'ನಮಗೆ ತಿಳಿದಿರಬೇಕಾದ ಯಾವುದೇ ಆರೋಗ್ಯ ಸಮಸ್ಯೆ ಇದೆಯೆ?',
            monthlyLeave: 'ನಿಮಗೆ ಮಾಸಿಕ ರಜೆ ಬೇಕೇ? ಇದ್ದರೆ ಎಷ್ಟು ದಿನ?',
            leavePreference: 'ರಜೆಯನ್ನು ಮಾಸಿಕ ಅಥವಾ ಒಟ್ಟಿಗೆ ತೆಗೆದುಕೊಳ್ಳಲು ಇಷ್ಟಪಡುತ್ತೀರಾ?',
            restTimeNeeded: 'ಕೆಲಸದ ವೇಳೆ ವಿಶ್ರಾಂತಿ ಸಮಯ ಬೇಕೇ? ಇದ್ದರೆ ಎಷ್ಟು ಹೊತ್ತು?',
            menstrualCycleComfort: 'ಮಾಸಿಕ ಧರ್ಮದ ಸಮಯದಲ್ಲಿ ಕೆಲಸ ಮಾಡಲು ಸಿದ್ಧರಿದ್ದೀರಾ? (ಐಚ್ಛಿಕ)',
            eatSameFood: 'ಮನೆಯಲ್ಲಿ ತಯಾರಿಸಿದ ಅದೇ ಆಹಾರ ತಿನ್ನಲು ಸಿದ್ಧರಿದ್ದೀರಾ?',
            vegNonVegPreference: 'ಮನೆಯ ಪ್ರಕಾರ ಶಾಕಾಹಾರಿ ಅಥವಾ ಮಾಂಸಾಹಾರಿ ಆಹಾರ ತಿನ್ನಲು ಸಿದ್ಧರಿದ್ದೀರಾ?',
            foodRestrictions: 'ಯಾವುದೇ ಆಹಾರ ನಿರ್ಬಂಧಗಳಿವೆಯೆ?',
            formalTraining: 'ಹೌಸ್ಕೀಪಿಂಗ್ ಅಥವಾ ಹಿರಿಯರ ಆರೈಕೆಗೆ ಸಂಬಂಧಿಸಿದ ತರಬೇತಿ ಪಡೆದಿದ್ದೀರಾ?',
            specificTrainingNeeded: 'ನಿಮಗೆ ಯಾವುದೇ ವಿಶೇಷ ತರಬೇತಿ ಬೇಕೇ?',
            openToTraining: 'ಅಗತ್ಯವಿದ್ದರೆ ಹೆಚ್ಚುವರಿ ತರಬೇತಿ ಪಡೆಯಲು ಸಿದ್ಧರಿದ್ದೀರಾ?',
            personalHabits: 'ಧೂಮಪಾನ, ಮದ್ಯ ಅಥವಾ ತಂಬಾಕು ಬಳಸುತ್ತೀರಾ?',
            followHouseRules: 'ಉದ್ಯೋಗದಾತರ ಮನೆಯ ನಿಯಮಗಳನ್ನು ಪಾಲಿಸಲು ಸಿದ್ಧರಿದ್ದೀರಾ?',
            consentText: 'ನಾನು ನೀಡಿದ ಎಲ್ಲಾ ಮಾಹಿತಿ ಸತ್ಯವಾಗಿದೆ. ಪರಿಶೀಲನೆ ಮತ್ತು ನೇಮಕಾತಿಗಾಗಿ ನನ್ನ ವೈಯಕ್ತಿಕ ಮಾಹಿತಿಯನ್ನು ಹಂಚಿಕೊಳ್ಳಲು ನಾನು ಒಪ್ಪುತ್ತೇನೆ.'
        }
    },
    ta: {
        label: 'தமிழ்',
        title: 'பணியாளர் நேர்காணல் கேள்வித்தாள்',
        sections: {
            basic: '1. அடிப்படை தகவல்கள்',
            workPrefs: '2. வேலை விருப்பங்கள்',
            habits: '3. வேலை பழக்கம் மற்றும் ஒழுக்கம்',
            health: '4. உடல்நலம் மற்றும் விடுப்பு',
            lifestyle: '5. உணவு மற்றும் வாழ்க்கை முறை',
            training: '6. பயிற்சி மற்றும் திறன்கள்',
            personal: '7. தனிப்பட்ட பழக்கங்கள்',
            consent: 'சட்ட ஒப்புதல்'
        },
        fields: {
            fullName: 'முழுப் பெயர்',
            age: 'வயது',
            maritalStatus: 'திருமண நிலை (திருமணமானவர் / திருமணமாகாதவர் / விதவை / விதவர்)',
            hasChildren: 'உங்களுக்கு குழந்தைகள் உள்ளனவா? இருந்தால் எண்ணிக்கை மற்றும் வயது',
            localReference: 'உங்களிடம் உள்ளூர் பரிந்துரை உள்ளதா? இருந்தால் தொடர்பு எண்',
            hasSmartphone: 'உங்களிடம் ஸ்மார்ட்போன் உள்ளதா?',
            speakHindi: 'நீங்கள் இந்தி பேச முடியுமா?',
            speakOtherLanguages: 'நீங்கள் ஆங்கிலம் அல்லது வேறு மொழி பேச முடியுமா? இருந்தால் குறிப்பிடவும்',
            readWriteHindi: 'நீங்கள் இந்தி படிக்கவும் எழுதவும் முடியுமா?',
            workInterests: 'நீங்கள் எந்த வகை வேலை செய்ய விரும்புகிறீர்கள்? (சமையல் / வீட்டு சுத்தம் / குழந்தை பராமரிப்பு / முதியோர் பராமரிப்பு / அனைத்து வேலைகள்)',
            comfortableCooking: 'சமையல் செய்ய உங்களுக்கு வசதியா?',
            cookType: 'சைவம், அசைவம் அல்லது இரண்டையும் சமைக்க முடியுமா?',
            prepareMilletDietary: 'சிறுதானிய அல்லது டயட் உணவு செய்ய முடியுமா?',
            comfortableHousekeeping: 'வீட்டு சுத்தம் செய்ய வசதியா?',
            willingCleanBathrooms: 'குளியலறை சுத்தம் செய்ய தயாரா?',
            knowIroning: 'உடைகள் இஸ்திரி செய்ய தெரியுமா?',
            operateWashingMachine: 'வாஷிங் மெஷின் பயன்படுத்த தெரியுமா?',
            useAppliances: 'மைக்ரோவேவ், மிக்சர், ஓவன் அல்லது ஏர் ஃப்ரையர் பயன்படுத்த தெரியுமா?',
            comfortableKids: 'குழந்தைகளை கவனிக்க வசதியா?',
            comfortableElderly: 'முதியவர்களை கவனிக்க வசதியா?',
            dropChildrenBusStop: 'குழந்தைகளை பஸ் ஸ்டாப் வரை அழைத்துச் செல்ல முடியுமா?',
            canDrive: 'கார் அல்லது இருசக்கர வாகனம் ஓட்ட முடியுமா?',
            wakeUpTime: 'நீங்கள் பொதுவாக எத்தனை மணிக்கு எழுகிறீர்கள்?',
            earlyStart: 'காலை நேரத்தில் வேலை தொடங்க வசதியா?',
            comfortablePets: 'செல்லப்பிராணிகள், குறிப்பாக நாய்களுடன் வசதியா?',
            nonVegHome: 'அசைவம் சமைக்கும் வீட்டில் வேலை செய்ய வசதியா?',
            phoneDuringWork: 'வேலை நேரத்தில் மொபைல் பயன்படுத்துகிறீர்களா?',
            socialMediaWork: 'வேலை நேரத்தில் சமூக ஊடகம் பயன்படுத்துகிறீர்களா?',
            manageMultipleTasks: 'ஒரே நேரத்தில் பல வேலைகளை கையாள முடியுமா?',
            runBehindChildren: 'தேவையானால் குழந்தைகளை துரத்தி கவனிக்க முடியுமா?',
            regularMedicines: 'நீங்கள் தொடர்ந்து எந்த மருந்துகளையும் எடுத்துக் கொள்கிறீர்களா?',
            healthConditions: 'எங்களுக்கு தெரிந்திருக்க வேண்டிய உடல்நல பிரச்சனை உள்ளதா?',
            monthlyLeave: 'மாதாந்திர விடுப்பு தேவையா? இருந்தால் எத்தனை நாட்கள்?',
            leavePreference: 'விடுப்பை மாதந்தோறும் அல்லது ஒரே முறையாக எடுக்க விரும்புகிறீர்களா?',
            restTimeNeeded: 'வேலை நேரத்தில் ஓய்வு நேரம் தேவையா? இருந்தால் எவ்வளவு நேரம்?',
            menstrualCycleComfort: 'மாதவிடாய் காலத்தில் வேலை செய்ய வசதியா? (விருப்பம்)',
            eatSameFood: 'வீட்டில் தயாரிக்கப்பட்ட அதே உணவை சாப்பிட வசதியா?',
            vegNonVegPreference: 'வீட்டின் வழக்கப்படி சைவம் அல்லது அசைவம் சாப்பிட வசதியா?',
            foodRestrictions: 'உணவு தொடர்பான எந்த கட்டுப்பாடுகள் உள்ளனவா?',
            formalTraining: 'வீட்டு வேலை அல்லது முதியோர் பராமரிப்பில் பயிற்சி பெற்றுள்ளீர்களா?',
            specificTrainingNeeded: 'உங்களுக்கு சிறப்பு பயிற்சி தேவையா?',
            openToTraining: 'தேவையானால் கூடுதல் பயிற்சி பெற தயாரா?',
            personalHabits: 'புகைபிடித்தல், மது அல்லது புகையிலை பயன்படுத்துகிறீர்களா?',
            followHouseRules: 'வீட்டின் விதிகளை பின்பற்ற தயாரா?',
            consentText: 'நான் வழங்கிய அனைத்து தகவல்களும் உண்மை. சரிபார்ப்பு மற்றும் பணியமர்த்தலுக்காக எனது தகவல்களை பகிர ஒப்புக்கொள்கிறேன்.'
        }
    },
    te: {
        label: 'తెలుగు',
        title: 'హెల్పర్ ఇంటర్వ్యూ ప్రశ్నపత్రం',
        sections: {
            basic: '1. ప్రాథమిక సమాచారం',
            workPrefs: '2. పని అభిరుచులు',
            habits: '3. పని అలవాట్లు మరియు క్రమశిక్షణ',
            health: '4. ఆరోగ్యం మరియు సెలవులు',
            lifestyle: '5. ఆహారం మరియు జీవనశైలి',
            training: '6. శిక్షణ మరియు నైపుణ్యాలు',
            personal: '7. వ్యక్తిగత అలవాట్లు',
            consent: 'చట్టపరమైన సమ్మతి'
        },
        fields: {
            fullName: 'పూర్తి పేరు',
            age: 'వయస్సు',
            maritalStatus: 'వివాహ స్థితి (వివాహిత / అవివాహిత / విధవ / విధురుడు)',
            hasChildren: 'మీకు పిల్లలు ఉన్నారా? ఉంటే ఎంత మంది మరియు వారి వయస్సు ఎంత?',
            localReference: 'మీకు స్థానిక రిఫరెన్స్ ఉందా? ఉంటే ఫోన్ నంబర్ ఇవ్వండి',
            hasSmartphone: 'మీ వద్ద స్మార్ట్ఫోన్ ఉందా?',
            speakHindi: 'మీరు హిందీ మాట్లాడగలరా?',
            speakOtherLanguages: 'మీరు ఇంగ్లీష్ లేదా ఇతర భాష మాట్లాడగలరా? ఉంటే చెప్పండి',
            readWriteHindi: 'మీరు హిందీ చదవడం మరియు రాయడం వచ్చా?',
            workInterests: 'మీరు ఏ రకమైన పని చేయాలని కోరుకుంటున్నారు? (వంట / హౌస్కీపింగ్ / పిల్లల సంరక్షణ / వృద్ధుల సంరక్షణ / అన్ని పనులు)',
            comfortableCooking: 'వంట చేయడంలో మీకు సౌకర్యంగా ఉందా?',
            cookType: 'శాకాహారం, మాంసాహారం లేదా రెండింటిని వండగలరా?',
            prepareMilletDietary: 'మిల్లెట్ లేదా డైట్ ఫుడ్ చేయగలరా?',
            comfortableHousekeeping: 'ఇంటి శుభ్రత పనులు చేయగలరా?',
            willingCleanBathrooms: 'బాత్రూమ్ శుభ్రం చేయడానికి సిద్ధంగా ఉన్నారా?',
            knowIroning: 'బట్టలు ఐరన్ చేయడం తెలుసా?',
            operateWashingMachine: 'వాషింగ్ మెషిన్ ఉపయోగించగలరా?',
            useAppliances: 'మైక్రోవేవ్, మిక్సర్, ఓవెన్ లేదా ఎయిర్ ఫ్రయ్యర్ ఉపయోగించగలరా?',
            comfortableKids: 'పిల్లలను చూసుకోవడంలో సౌకర్యంగా ఉందా?',
            comfortableElderly: 'వృద్ధులను చూసుకోవడంలో సౌకర్యంగా ఉందా?',
            dropChildrenBusStop: 'పిల్లలను బస్ స్టాప్కి తీసుకెళ్లగలరా?',
            canDrive: 'కారు లేదా ద్వీచక్ర వాహనం నడపగలరా?',
            wakeUpTime: 'మీరు సాధారణంగా ఎంత గంటలకు లేస్తారు?',
            earlyStart: 'ఉదయం తొందరగా పని ప్రారంభించడానికి సిద్ధంగా ఉన్నారా?',
            comfortablePets: 'పెట్స్, ముఖ్యంగా కుక్కలతో సౌకర్యంగా ఉన్నారా?',
            nonVegHome: 'మాంసాహారం వండే ఇంట్లో పని చేయగలరా?',
            phoneDuringWork: 'పని సమయంలో మొబైల్ ఫోన్ ఉపయోగిస్తారా?',
            socialMediaWork: 'పని సమయంలో సోషల్ మీడియా ఉపయోగిస్తారా?',
            manageMultipleTasks: 'ఒకేసారి అనేక పనులు నిర్వహించగలరా?',
            runBehindChildren: 'అవసరమైతే పిల్లల వెనుక పరిగెత్తగలరా?',
            regularMedicines: 'మీరు ఏవైనా నియమిత మందులు తీసుకుంటున్నారా?',
            healthConditions: 'మాకు తెలియవలసిన ఆరోగ్య సమస్య ఏదైనా ఉందా?',
            monthlyLeave: 'నెలవారీ సెలవులు అవసరమా? ఉంటే ఎన్ని రోజులు?',
            leavePreference: 'సెలవులను నెలవారీగా లేదా ఒకేసారి తీసుకోవాలనుకుంటున్నారా?',
            restTimeNeeded: 'పని సమయంలో విశ్రాంతి అవసరమా? ఉంటే ఎంతసేపు?',
            menstrualCycleComfort: 'మాసిక ధర్మ సమయంలో పని చేయడానికి సౌకర్యంగా ఉందా? (ఐచ్ఛికం)',
            eatSameFood: 'ఇంట్లో తయారైన అదే ఆహారం తినడానికి సౌకర్యంగా ఉందా?',
            vegNonVegPreference: 'ఇంటి ప్రకారం శాకాహారం లేదా మాంసాహారం తినగలరా?',
            foodRestrictions: 'ఏవైనా ఆహార పరిమితులు ఉన్నాయా?',
            formalTraining: 'హౌస్కీపింగ్ లేదా వృద్ధుల సంరక్షణలో శిక్షణ పొందారా?',
            specificTrainingNeeded: 'మీకు ప్రత్యేక శిక్షణ అవసరమా?',
            openToTraining: 'అవసరమైతే అదనపు శిక్షణ పొందడానికి సిద్ధంగా ఉన్నారా?',
            personalHabits: 'పొగ త్రాగడం, మద్యం లేదా పొగాకు ఉపయోగిస్తారా?',
            followHouseRules: 'ఇంటి నియమాలను పాటించడానికి సిద్ధంగా ఉన్నారా?',
            consentText: 'నేను ఇచ్చిన సమాచారం నిజమైనది. ధృవీకరణ మరియు నియామక ప్రయోజనాల కోసం నా వ్యక్తిగత వివరాలను పంచుకోవడానికి నేను సమ్మతిస్తున్నాను.'
        }
    }
};
